...
ERROR_MSG_START = "Starting of data generation failed."
class DataFlowRaiseError(model.DataFlow):
    # very basic dataflow which raises an error during the start of the data generation
    def __init__(self, *args, **kwargs):
        model.DataFlow.__init__(self, *args, **kwargs)
        self._thread_must_stop = threading.Event()
        self._thread = None
    ...
    def start_generate(self):
        ...
        raise Exception(ERROR_MSG_START)
...
class TestDataFlow(unittest.TestCase):
    ...
    def test_df_raise_error(self):
        ...
        with self.assertRaises(Exception) as ex:
            im = self.df.get()
        # check the error message was correctly forwarded
        self.assertEqual(str(ex.exception), ERROR_MSG_START)
